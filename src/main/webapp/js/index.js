(function ($) {
    ...
    $.learunindex = {
        ...
        loadMenu: function () {
            var _html = "";
            $.ajax({
                url:'/erp/Permission/queryLstPer',
                data:{},
                dataType:'json',
                type:'post',
                async:false,
                success:function(data){
                    $.each(data, function (i) {
                        ...
                        if (row.pid == "0") {
                            ...
                            _html += '<a href="#">'
                            _html += '<i class="' +"fa fa-desktop"+ '"></i><span>' + row.pername + '</span><i class="fa fa-angle-left pull-right"></i>'
                            _html += '</a>'
                            var childNodes = $.learunindex.jsonWhere(data, function (v) { return v.pid == row.perid });
                            if (childNodes.length > 0) {
                                _html += '<ul class="treeview-menu">';
                                $.each(childNodes, function (i) {
                                    var subrow = childNodes[i];
                                    var subchildNodes = $.learunindex.jsonWhere(data, function (v) { return v.pid == subrow.perid });
                                    _html += '<li>';
                                    if (subchildNodes.length > 0) {
                                        _html += '<a href="#"><i class="' + subrow.F_Icon + '"></i>' + subrow.pername + '';
