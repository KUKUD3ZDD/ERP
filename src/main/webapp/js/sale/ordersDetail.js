...
//初始化表格
   function initTable() {
       var bid=$("#bid").val(); //订单编号
        // 表格渲染
        table.render({
            elem: '#demo2',
            id:"depTableId",
            height: "full",
            where:{'orderid':bid}, //设定异步数据接口的参数
            url: projectName+'/ordersline/query', //数据接口
            page: false, //开启分页
            cols: [[ //表头
                {field: 'boid', title: '编号', width:100, sort: true, fixed: 'left',event:'setDetail'},
   ...
   function ordersLineDetail(data){
       ...
       layer.open({
           type: 2,
           content: 'page/sale/ordersLineDetail.jsp',
           //content:editDiv,
           title: '订单明细',
           maxmin: true,
           shade: 0,
           ...
           success: function(layero, index) {
               ...
               if (data) {
                   ...
                   body.find(".bid").val(data.orderid);
